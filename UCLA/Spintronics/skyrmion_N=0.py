...
# Axisymmetric "skyrmion-like" profile for theta(r) from the Belavin–Polyakov form:
...
# General texture:
# n(x,y) = (sinθ(r) cosφ, sinθ(r) sinφ, cosθ(r))
# with φ = W*α + γ (W integer gives vorticity). Set W=0 to make vortex number 0.
def n_texture(x, y, lam=1.0, W=1, gamma=0.0):
    r = np.sqrt(x*x + y*y)
    alpha = np.arctan2(y, x)  # azimuthal angle in real space
    sin_th, cos_th = theta_profile(r, lam=lam)
    phi = W * alpha + gamma   # in-plane angle
...
if __name__ == "__main__":
    ...
    # --- Set W=0 to make vortex number zero, which forces skyrmion number ~0 ---
    W = 0
    gamma = 0.0  # constant in-plane direction; can be anything
    ...
    print(f"Measured vortex number W ≈ {W_meas:.6f} (target 0)")
    print(f"Measured skyrmion number N ≈ {N_meas:.6f} (target 0)")
