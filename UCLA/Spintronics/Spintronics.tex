...
Another key observation here is that, even we employed the Coulomb gauge in the derivation from (**) to (***), we should note that then quantity $K$ defined via (***) is independent on the gauge choice as long as the boundary condition satisfies ($\dagger$).  To see this, given that ($\dagger$) holds, consider $\vec{A} \to \vec{A} + \nabla f$, then
\begin{align*}
...
In this story, we said that the emergent field $\vec{\mathcal{B}}$ has field lines formed by the preimaged loops. To verify this though, consider parametrizing 
$$\vec{n}(\vec{r}) = \bmat{\sin(\theta(\vec{r}))\, \cos(\varphi(\vec{r}))\\ \sin(\theta(\vec{r}))\,\sin(\varphi(\vec{r}))\\ \cos(\theta(\vec{r})) 
} \in S^2\,,$$
then we see that \footnote{Geometrically, this is the pullback of the area element on the unit sphere
\begin{align*}
d\Omega = \sin(\theta)\,d\theta \wedge d\varphi\,.
\end{align*}}
\begin{align*}
\mathcal{B}_i\sim \vec{n}\cdot(\pd_j \vec{n}\times \pd_k\vec{n}) = \sin(\theta) \left(\pd_i\theta\,\pd_j\varphi - \pd_j\theta\,\pd_i\varphi\right)\,.
\end{align*}
Thus from here we can write
\begin{align*}
\vec{\mathcal{B}}\sim \sin(\theta)\,\nabla\theta \times \nabla \varphi\,.
\end{align*}
If we pick $\vec{n}_0\in S^2$, this is a process of fixing $\theta = \theta_0$ and $\varphi = \varphi_0$. Thus the preimage $\vec{n}^{-1}(\vec{n}_0)$ is the intersection of two level surfaces $\theta(\vec{r}) = \theta_0$ and $\varphi(\vec{r}) = \varphi_0$, and the direction tangent to the intersection curve is $\nabla \theta \times \nabla \varphi$, which is exactly the direction of $\vec{\mathcal{B}}$ as we saw above. This argument here also reveals that the each field line of $\vec{\mathcal{B}}$ traces out constant $\vec{n}$.  Notice that, the argument above requires a locally non-vanishing $\vec{\mathcal{B}}$. For the locations where $\vec{\mathcal{B}}= 0$, the field line direction is undefined, but the direction of $\vec{n}$ is still well defined. 
...
\subsection*{Hopfion viewed as twisted Skyrmion tubes}
Perhaps one of the simplest (in the geometrical sense) hopfions is the one formed by a twisted tube of skyrmions. Let us start with a single 


\chapter{-}
\section{Landau-Lifshitz-Gilbert Equation}
The Landau-Lifshitz-Gilbert equation (LLG equation) is a fundamental equation in the field of magnetism that describes the dynamics of magnetization in magnetic materials. For a single magnetic moment $\vec{M}$, the full form of LLG equation reads
\begin{align*}
\frac{d\vec{m}}{dt} = -\gamma \vec{m}\times \vec{H}_{\text{eff}} + \alpha \vec{m}\times \frac{d\vec{m}}{dt}\,,\qquad
\text{where } \vec{m} = \frac{\vec{M}}{M_\text{S}}\,.\tag{*}
\end{align*}
where $M_{\text{S}}$ is the saturation magnetization when describing the total magnetization of a ferromagnetic material. The LHS of (*) describes the total rate of change with respect to time, due to the torque given by the RHS. \\


The first term in torque,
$$-\gamma\vec{m} \times \vec{H}_{\text{eff}}\,,$$ 
is an analog of what we have seen in section 1.3.1 Larmor precession, and is called the conservative torque due to an effective field $H_{\text{eff}}$. 
The effective field $\vec{H}_{\text{eff}}$ is required to be conservative such that it has the form 
\begin{align*}
\vec{H}_{\text{eff}} = -\frac{1}{\mu_0 M_S}\frac{\pd E}{ \pd\vec{m}}
\end{align*}
of an energy scalar field $E$. As we have seen in the Larmor precession section, this term will make $\vec{m}$ precess around the direction of $H_{\text{eff}}$ without damping. For the energy functional 
\begin{align*}
E = -\mu_0 M_S \vec{m}\cdot \vec{H}_{\text{ext}}
\end{align*}
due to the interaction between the magnetic moment and an external field $\vec{H}_{\text{ext}}$, we see easily that we have $\vec{H}_{\text{ext}} = \vec{H}_{\text{eff}}$. \\


The second term in torque, 
\begin{align*}
\alpha \vec{m}\times \frac{d\vec{m}}{dt}\,,
\end{align*}
called the Gilbert damping term, is dissipative and it represents the (phenomenological) effect of damping in the system. $\alpha$ is called the Gilbert damping coefficient which characterizes the rate at which energy is lost as the magnetization evolves over time. The dissipative torque ensures that the magnetization eventually reaches a stable state aligning with $\vec{H}_{\text{eff}}$ and it arises from the interaction between the magnitization of the magnetic material and its surrounding environment. It could be understood as the exchange of angular momentum between the magnetization and other degrees of freedom in the material. 

\subsection{Spin transfer torque mechanism}
There are other torques that can be added to the LLG equation (*), one of such is the spin transfer torque (STT). The picture of STT is the following: We start with a spin-unpolarized current, that is, no net spin polarization exists in the current. As the current passes through a fixed ferromagnetic layer, the current then get polarized in the direction of magnetization of the ferromagnet. The flow of spin polarized electrons is then allowed to flow into a second, free and misaligned, ferromagnetic layer, which results in a transfer of spin angular momentum to the local magnetization via exchange interactions. The exchange of spin angular momentum causes dynamics in the free layer. \\

To mathematically make the STT picture concrete, we need to start with deriving the continuity equation for charge and spin density. The electron number density, or probability density, at a given spatial location, is 
\begin{align*}
n(\vec{r}) = \sum_{\text{spin }s} \psi_s^*(\vec{r}) \,\psi_s(\vec{r})\,,
\end{align*}
with $\psi_s$ being the wavefunction of electron with spin $s$. From here we have the continuity equation for electron flow
\begin{align*}
\frac{\pd n}{\pd t} = -\nabla \cdot \vec{J} \,,
\end{align*}
with $\vec{J}$ being the probability current density,
\begin{align*}
\vec{J} = \frac{\hbar}{m} \sum_{\text{spin }s} \Im\left( 
\psi_s^*\, \nabla \psi_s
\right)
\end{align*}
Since each electron carries a scalar-valued quantum of charge, conservation of electron number is equivalent to the conservation of charge. Now we will want to incorporate the electron spin into a similar conservation law. Let us start by electronic spin-$z$ density
\begin{align*}
s_z(\vec{r}) = \frac{\hbar}{2}\left( \psi_{\uparrow}^*(\vec{r}) \,\psi_{\uparrow}(\vec{r}) - \psi_{\downarrow}^*(\vec{r})\,\psi_{\downarrow}(\vec{r})\right)\,.
\end{align*}
This is intuitive, but to account for spin in other directions, a generalization is required, 
\begin{align*}
\vec{s}(\vec{r}) = \sum_{s,s'\in \{\uparrow,\downarrow\}} \psi_s^*(\vec{r}) \, \frac{\hbar}{2}\, \vec{\sigma}_{ss'}\, \psi_{s'}(\vec{r})\,,
\end{align*}
where 
\begin{align*}
\vec{\sigma}_{ss'} = \bmat{(\sigma_x)_{ss'}\\
(\sigma_y)_{ss'}\\
(\sigma_z)_{ss'}
}
\end{align*}
for Pauli matrices $\sigma_x,\,\sigma_y,\,\sigma_z$. The spin density $\vec{s}$ defined here is measured in the $z$-direction basis. If the electron is locally in the pure spin-up state, 
\begin{align*}
\vec{\psi}(\vec{r}) = \bmat{\phi(\vec{r})\\0}\,,\qquad
\vec{s}(\vec{r}) = \frac{\hbar}{2}|\phi(\vec{r})|^2\,\hat{z}\,.
\end{align*}
We also notice that $\vec{s}$ is a vector, and thus the mathematical structure of its conservation law should be fundamentally different from that of conservation law for scalar quantities. Define the spin current density as $\vec{Q}$, we shall have
\begin{align*}
\frac{\pd \vec{s}}{\pd t} = -\nabla \cdot \vec{Q}\,,
\end{align*}
where 
\begin{align*}
\vec{Q} = \frac{\hbar}{m}\sum_{s,s'\in \{\uparrow,\,\downarrow\}} \Im\left(
\psi_s^*\,\frac{\hbar}{2}\,\vec{\sigma}_{ss'}\otimes \nabla\psi_{s'}
\right) \,.
\end{align*}
We note here $\vec{Q}$ is tensor-valued,
\begin{align*}
\vec{Q} \sim \text{(Spin)}\otimes \text{(Velocity)}\,.
\end{align*}
Now we focus our study to the flow of spin current in a ferromagnetic material layer where the magnetization $\vec{M}$ of the layer is fixed. In the ferromagnet, there is a intrinsic exchange field, produced by the magnetization $\vec{M}$, coupling the spin from spin current and the spin in the magnet. Such an exchange field has no effect on the charge continuity equation because it does not couple to the charge degree of freedom, but it does modify the spin continuity equation for the spin current,
\begin{align*}
\frac{\pd \vec{s}}{\pd t} = -\nabla \cdot\vec{Q} - \gamma_{\text{ex}} \vec{s}\times \vec{B}_{\text{ex}}\, \tag{$\dagger$}
\end{align*}
with $\vec{B}_{\text{ex}}$ being the exchange field and $\vec{s}$ being the spin in the spin current flow. \\

In the steady state, conduction electrons are still flowing through the magnet, but the spatial profiles of the charge and spin densities (in the spin current) have settled into time-independent form, that is $\pd n/\pd t = 0$ and $\pd\vec{s}/\pd t = 0$, we then expect to see
\begin{align*}
\nabla \cdot \vec{J} = 0\,,\qquad\vec{J}_{\text{in}} = \vec{J}_{\text{out}}
\end{align*}
for the charge in the spin current, but due to ($\dagger$) we have
\begin{align*}
\nabla \cdot \vec{Q} = -\gamma_{\text{ex}} \vec{s} \times \vec{B}_{\text{ex}}\,,\qquad
\vec{Q}_{\text{in}}\neq \vec{Q}_{\text{out}} \tag{$\ddagger$}
\end{align*}
for the spin in the spin current. The changing of $\vec{Q}$ in this process is understood as having angular momentum deposited to the bulk of the magnet, as we will discuss later on. We also notice that, from the structure of ($\dagger$), the exchange field term $-\gamma_{\text{ex}} \vec{s}\times \vec{B}_{\text{ex}}$ provides a torque perpendicular to the direction of $\vec{s}$. If we suppose
\begin{align*}
\vec{Q} \sim \vec{s}\otimes (v_x \hat{x})
\end{align*}
then ($\ddagger$) tells us that 
\begin{align*}
v_x \,\frac{d\vec{s}}{dx} \sim -\gamma_{\text{ex}} \vec{s}\times \vec{B}_{\text{ex}}\,,
\end{align*}
from which we see that the profile $\vec{s}$ spatially precesses around the direction of $\vec{B}_{\text{ex}}$. This argument is in fact not full story yet. Realistically, the spin current incidents the 2-dimensional interface in a 3-dimensional space, so many electrons are incident on the magnetic layer
from a variety of directions, corresponding to states from all
parts of the Fermi surface, and therefore different electrons
take different paths through the magnetic layer. Furthermore, the (spatial) period of the precession is very short for a typical transition metal ferromagnet, on the scale of a few atomic lattice
spacings. Then even if all of the electrons begin with perfectly aligned spins at the interface, electrons reaching a
given depth inside the magnet will have traveled different
path lengths to get there. The result is classical dephasing: All components in $\vec{s}$ transverse to the direction of $\vec{B}_{ex}$ cancel due to destructive interference. On the other hand, using argument of energetics, one expects to see that spin in the spin current that is parallel to $\vec{B}_{ex}$ is likely get transmitted and spin in the spin current that is antiparallel to $\vec{B}_{ex}$ is likely to get reflected. From either perspective, the spin current coming out from the ferromagnetic layer is therefore spin polarized in the direction of $\vec{B}_{ex}$ (assuming that $\vec{n}$ has fixed direction in the ferromagnetic layer). This mechanism is detailed with toy models on \textit{Spin Transfer Torques} by D.\,C.\,Ralph and M.\,D.\,Stiles. \\

\subsection{Spin transfer torque to LLG equation}
Now we have investigated how the fixed magnetization $\vec{M}$ in the ferromagnet and the resulting exchange field $\vec{B}_{\text{ex}}$ exert torques on the spin current, but there should be a back torque by the spin current acting on $\vec{M}$ due to this effect. Due to conservation of total spin angular momentum in the system, the torque and back torque should be in equal magnitude but opposite directions. As we argued above, the exchange torque on the conduction electrons by the magnetization is 
\begin{align*}
\tau_{m \to e} = -\gamma_{\text{ex}}\vec{s}\times \vec{B}_{\text{ex}}\,,
\end{align*}
thus the back torque on the magnetization is 
\begin{align*}
\tau_{e\to m} = \gamma_{\text{ex}} \vec{s}\times \vec{B}_{\text{ex}} = -\nabla \cdot \vec{Q} - \frac{\pd \vec{s}}{\pd t}\,.
\end{align*}
Continuing the story, if the spin polarized current now passes through a second free layer of ferromagnet, there is a torque acting on the magnetization texture $\vec{m}$ in this second layer, and such a torque is called the spin transfer torque, which is a new term that can be added to the RHS of the LLG equation (*). First we consider the limit where there is a strong exchange field between the spin current and the magnetization, with the magnetization being slowly varying in space. In this picture, the spin profile $\vec{s}(\vec{r}) $ of the spin current adjusts fast enough to align roughly with the magnetization texture (locally, $\vec{s}(\vec{r})$ is precessing around $\vec{m}(\vec{r})$ fast enough such that when $\vec{m}$ varies slowly in space, $\vec{s}$ follow the variation of $\vec{m}$) such that locally we have 
\begin{align*}
s(\vec{r}) \sim \vec{m}(\vec{r})\,,
\end{align*}
in which case, in the steady state, by direct computation,
\begin{align*}
\tau_{\text{STT,\ ad}}= - \nabla \cdot \vec{Q} \sim -(\vec{v}\cdot \nabla) \vec{m}\,,
\end{align*}
with $\vec{v}$ being the velocity of the spin current. This is the back torque acting on the magnetization of the magnet, and is called the adiabatic term in spin transfer torque. Notice here, if the texture $\vec{m}$ is uniform, the quantity $\tau_{\text{STT,\ ad}}$ is necessarily zero, because, in that case, the adiabatic limit of $\vec{s}(\vec{r}) \sim \vec{m}(\vec{r})$ at any point implies global alignment of the two angular momenta, and there will be minimal exchange interaction ($H_{\text{ex}} \sim -\vec{s} \cdot \vec{M}$ is minimized) and thus angular momentum exchange will not take place. The contribution of such an adiabatic limit relies on a non-uniform texture $\vec{m}$. For notation, we will write
\begin{align*}
\tau_{\text{STT,\ ad}} = -(\vec{u}\cdot \nabla) \vec{m}\,,
\end{align*}
where one can find the coefficient $u_0$ in $\vec{u} = u_0 \vec{v}$ using a more careful calculation. Moreover, we notice that $\tau_{\text{STT,\ ad}}$ has a mathematical form of advection, transport of a field by a velocity $\vec{u}$. The operator $\pd_t + \vec{u}\cdot \nabla$ is exactly the material derivative used in fluid mechanics, which describes how a field changes when it is being carried along by a flow with velocity $\vec{u}$. The term $\tau_{\text{STT,\ ad}}$ looks like the spin current is advecting the magnetization texture.\\

The other spin transfer torque term is a correction term to the adiabatic term, 
\begin{align*}
\tau_{\text{STT,\ non-ad}} = \beta \vec{n}\times (\vec{v}\cdot \nabla) \vec{m}\,,
\end{align*}
with some $\beta$ constant. This term can be derived from the component of $\vec{s}(\vec{r})$ that is transverse to $\vec{m}(\vec{r})$. More rigorous derivations and experimental justification of these terms can be found in S.\ Zhang and Z.\ Li's work on \textit{Roles of Nonequilibrium Conduction Electrons on the Magnetization Dynamics of Ferromagnets} (PRL 2004), \textit{Domain-Wall Dynamics and Spin-Wave Excitations with Spin-Transfer Torques} (PRL 2004), and A. Thiaville, Y. Nakatani, J. Miltat, and Y. Suzuki's work on \textit{Micromagnetic understanding of current-driven domain wall motion in patterned nanowires} (EPL 2005). \\

In summary, we obtain the extended LLG equation with the spin transfer torque terms due to a spin current passing through the magnet, 
\begin{align*}
\frac{d\vec{m}}{dt} = -\gamma \vec{m}\times \vec{H}_{\text{eff}} + \alpha \vec{m}\times \frac{d\vec{m}}{dt}-(\vec{u}\cdot \nabla) \vec{m} +\beta \vec{n}\times (\vec{v}\cdot \nabla) \vec{m}\,.
\end{align*}
