...
\begin{align*}
\frac{\pd \vec{s}}{\pd t} = -\nabla \cdot\vec{Q} - \gamma \vec{s}\times \vec{B}_{\text{ex}}\, \tag{$\dagger$}
\end{align*}
...
\begin{align*}
\nabla \cdot \vec{Q} = -\gamma \vec{s} \times \vec{B}_{\text{ex}}\,,\qquad
\vec{Q}_{\text{in}}\neq \vec{Q}_{\text{out}} \tag{$\ddagger$}
\end{align*}
for the spin in the spin current. The changing of $\vec{Q}$ in this process is understood as having angular momentum deposited to the bulk of the magnet, as we will discuss later on. We also notice that, from the structure of ($\dagger$), the exchange field term $-\gamma \vec{s}\times \vec{B}_{\text{ex}}$ provides a torque perpendicular to the direction of $\vec{s}$. If we suppose
\begin{align*}
\vec{Q} \sim \vec{s}\otimes (v_x \hat{x})
\end{align*}
then ($\ddagger$) tells us that 
\begin{align*}
v_x \,\frac{d\vec{s}}{dx} \sim -\gamma \vec{s}\times \vec{B}_{\text{ex}}\,,
\end{align*}
